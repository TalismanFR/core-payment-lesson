...
func main() {
	...
	v, err := NewVaulat(addr, mountPath)
	if err != nil {
...
func (v Vault) Initialize(out io.Writer) error {
	...
	if isInit {
		panic("vault shouldn't have been initialized before")
	}
...
func (v *Vault) Populate(in io.ReadCloser) error {
	defer in.Close()
	...
	if len(m) == 0 {
		return fmt.Errorf("no terminals in file")
	}

	for alias, data := range m {
		creds, ok := data.(map[string]interface{})
		if !ok {
			return fmt.Errorf("wrong data type: expect %T, got: %T", creds, data)
		}

		creds["alias"] = alias

		_, err := v.c.KVv2(v.mountPath).Put(ctx, uuid.New().String(), creds)
		if err != nil {
